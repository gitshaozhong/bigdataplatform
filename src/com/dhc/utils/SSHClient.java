...
public class SSHClient {
	...
	private Expect4j SSH() throws Exception {
		...
		//System.out.println("in SSH connect");
		session.connect(60000);
		ChannelShell channel = (ChannelShell) session.openChannel("shell");
		Expect4j expect = new Expect4j(channel.getInputStream(), channel.getOutputStream());
		channel.connect();
		//System.out.println("after SSH connect");
		return expect;
	}
	/**
	 *
	...
	/*public static void main(String[] args) {
		new Thread() { // 一个匿名的线程类
            ...
            public void run() {
				String ip = AppConstants.getSSHRemoteHost();
				String user =AppConstants.getSSHUserName();
				String password = AppConstants.getSSHPassword();
				
				SSHClient ssh = new SSHClient(ip, user, password);
